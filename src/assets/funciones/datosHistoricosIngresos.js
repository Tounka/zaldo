...
export const cargarHistoricosEnFirestore = async (uid) => {
    ...
    const data2025 = doc2025Actual
        ? {
            ...doc2025Actual,
            empresas: combinarSinDuplicados(doc2025Actual.empresas, DATA_HISTORICA_2025.empresas),
            registros: combinarSinDuplicados(doc2025Actual.registros, DATA_HISTORICA_2025.registros),
        }
        : DATA_HISTORICA_2025;
    ...
    const data2026 = doc2026Actual
        ? {
            ...doc2026Actual,
            empresas: combinarSinDuplicados(doc2026Actual.empresas, DATA_HISTORICA_2026.empresas),
            registros: combinarSinDuplicados(doc2026Actual.registros, DATA_HISTORICA_2026.registros),
        }
        : DATA_HISTORICA_2026;
...
const combinarSinDuplicados = (existentes = [], nuevos = []) => {
    const map = new Map();
    (existentes || []).forEach((item) => map.set(item.id || item.nombre, item));
    (nuevos || []).forEach((item) => {
